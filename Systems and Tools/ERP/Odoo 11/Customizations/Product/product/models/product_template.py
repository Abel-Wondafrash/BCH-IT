...
class ProductTemplate(models.Model):
    ...
    def _get_default_uom_sale_id(self):
        ...
        if pack_uom:
            ...
            # If 'Pack' doesn't exist, use the existing default method
            return self._get_default_uom_id()

    name = fields.Char('Name', index=True, required=True, translate=True)
    code = fields.Char('Code')
    shorthand_name = fields.Char( 'Shorthand Name', index=True, help='A short alternative name used in reports.',size=20)
    ...
    product_pack_qty = fields.Float('Pack Quantity')
    weight = fields.Float(
    ...
    uom_sale_id = fields.Many2one(
        'product.uom', 'Sales Unit of Measure',
    ...
    def create(self, vals):
        product_with_name= self.env['product.template'].search([('name','=',vals['name'])])
        if product_with_name:
            raise UserError('Product with name Exists')
        ...
        related_vals = {}
        code="ITM-00"
        pro_name=code+ str(template.id)
        related_vals['code'] =str(pro_name)
