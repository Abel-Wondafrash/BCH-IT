...
class accountReconcilation(models.Model):
    _name = 'reconcile.account.move'
   
    move_id = fields.Many2one('account.move', string='Journal' )
    name = fields.Many2one('account.move', string='Journal' )
    ...
    def _onchange_name(self):
        debit_amount=0
        _logger.info('-------name******** = %s',self.move_id)
        move_line_ids= self.env['account.move.line'].search([('move_id', '=', self.move_id.id)]).ids
        _logger.info('-------move_line_ids******** = %s',move_line_ids)
        for move_line_id in self.env['account.move.line'].browse(move_line_ids):
            debit_amount=debit_amount+ move_line_id.debit
        _logger.info('-------debit_amount******** = %s',debit_amount)
...
class AccountMove(models.Model):
    ...
    is_reconciled = fields.Boolean('Reconciled',default=False)
    is_reconcile_checked = fields.Boolean('Reconcile Checked',default=False)
    employee_id = fields.Many2one('hr.employee', string='Employee' )
    partner = fields.Many2one('res.partner', string='Partner')
    name = fields.Char(string='Number', required=True, copy=False, default='/')
    case = fields.Selection([('purchase', 'Local Purchase'), 
    ('forign_purchase', 'Foriegn Purchase'),('deposit','Deposit')],string='Case')
    main_id = fields.Integer(string='Main Id')
    ref = fields.Char(string='Source Document', copy=False)
    ...
    amount = fields.Monetary(compute='_amount_compute', store=True, digits = (12,6) )
    narration = fields.Text(string='Internal Note')
    amount_val = fields.Float(string='Amount')
    reconcile_amount_val = fields.Float(string='Rconcile Amount')
    company_id = fields.Many2one('res.company', 
    related='journal_id.company_id', string='Company', store=True, readonly=True,
    ...
    reconcile_line_ids = fields.One2many('reconcile.account.move', 'name', string='Journal Items')
    

    @api.onchange('reconcile_line_ids')
    def _onchange_reconcile_line_ids(self):
        reconcile_total_amount=0.0
        if self.reconcile_line_ids:
            reconciles = self.env['reconcile.account.move'].search([('name', '=', self.id)]).ids
                    
            _logger.info('-------move_det************ = %s',reconciles)
            if reconciles:
                for reconcile in self.env['reconcile.account.move'].browse(reconciles):
                    
                    move_line_ids= self.env['account.move.line'].search([('move_id', '=', reconcile.move_id.id)]).ids
                    for move_line_id in self.env['account.move.line'].browse(move_line_ids):
                        debit_amount=move_line_id.debit
                        reconcile_total_amount=reconcile_total_amount+debit_amount
            
            self.reconcile_amount_val=reconcile_total_amount
    def account_reconcile(self):
	
        self.is_reconciled =True
    def remove_reconcile(self):
        self.is_reconciled =False
        self.is_reconcile_checked =False
    def check_reconcile(self):
	
        main_total_amount=0.0
        reconcile_total_amount=0.0
        if self.reconcile_line_ids:
            reconciles = self.env['reconcile.account.move'].search([('name', '=', self.id)]).ids
            ...
            if reconciles:
                for reconcile in self.env['reconcile.account.move'].browse(reconciles):
                    ...
                    for move_line_id in self.env['account.move.line'].browse(move_line_ids):
                        debit_amount=move_line_id.debit
                        reconcile_total_amount=reconcile_total_amount+debit_amount
            move_line_ids= self.env['account.move.line'].search([('move_id', '=', self.id)]).ids
            for move_line_id in self.env['account.move.line'].browse(move_line_ids):
                debit_amount=move_line_id.debit
                main_total_amount=main_total_amount+debit_amount
                       
                    
            _logger.info('-------main_total_amount******** = %s',main_total_amount)
            _logger.info('-------reconcile_total_amount************ = %s',reconcile_total_amount)
            self.reconcile_amount_val=reconcile_total_amount
            if main_total_amount != reconcile_total_amount:
                raise UserError(_('Please verify again reconcilation amount and journal amount do not matched'))
            else:
                self.is_reconcile_checked =True
    # @api.multi
    ...
    def create(self, vals):
        ...
        _logger.info('-------!!!!!!!!create account.move vals=%s',vals)
    ...
    def post(self):
        ...
        for move in self:
            ...
            if move.name == '/':
                ...
                if new_name:
                    move.name = new_name
        # self.message_post( body='<p><b> Journa Posted</p>')
...
class AccountMoveLine(models.Model):
    ...
    @api.depends('move_id')
    def _compute_lable(self):
        for loop in self: 
            if loop.move_id:
                _logger.info('-------self.move_id************ = %s',loop.move_id)
                _logger.info('-------self.move_id.employee_id************ = %s',loop.move_id.employee_id)
                if loop.move_id.employee_id:
                    _logger.info('-------move_id.employee_id.name************ = %s',loop.move_id.employee_id.name)
                    loop.name=loop.move_id.employee_id.name
    ...
    debit = fields.Monetary( currency_field='company_currency_id', digits=dp.get_precision('Product Unit of Measure') )
    credit = fields.Monetary( currency_field='company_currency_id', digits=dp.get_precision('Product Unit of Measure'))
    ...
    name = fields.Char(string="Label",compute='_compute_lable')
    ...
    def create(self, vals):
        ...
        if vals.get('credit', 0.0) > 0:
            value["amount"]=vals.get('credit', 0.0)
        else:
            value["amount"]=vals.get('debit', 0.0)
        # value["reference"]=vals.get('reference')
        _logger.info('-------vals**ref********** = %s', vals.get('ref'))
        # if not vals.get('ref'):
        #     vals["ref"]=vals.get('name')
        value["cost_category"]=vals.get('cost_category')
        _logger.info('-------amount************ = %s',vals.get('credit', 0.0))
        _logger.info('-------reference************ = %s',vals.get('reference'))
        _logger.info('-------cost_category************ = %s',vals.get('cost_category'))
        context = dict(self._context or {})
        _logger.info('-------context************ = %s',context)
        amount = vals.get('debit', 0.0) - vals.get('credit', 0.0)
        
        if not vals.get('partner_id') and context.get('partner_id')and not vals["ref"]:
            vals['partner_id'] = context.get('partner_id')
        move = self.env['account.move'].browse(vals['move_id'])
        _logger.info('-------move.ref************ = %s',move.ref)
        
        move_det = self.env['cost.build.up'].search([('move_id', '=', move.id)])
        
        _logger.info('-------move_det************ = %s',move_det)
        if not move_det:
            value["source_document"]=move.ref
            value["move_id"]=move.id
            self.env['cost.build.up'].create(value)
    ...
    def write(self, vals):
        ...
        _logger.info('-------vals************ = %s',vals)
        if vals.get('credit', 0.0) > 0:
            amount=vals.get('credit', 0.0)
        else:
            amount=vals.get('debit', 0.0)
        ...
        if amount >0:
            for record in self:
                ...
                # move = self.env['account.move'].browse(record.id)
                move = self.env['account.move.line'].browse(record.id)
                if move:
                    _logger.info('-------move.ref************ = %s',move.ref)
                    
                    move_det = self.env['cost.build.up'].search([('move_id', '=', move.id)]).ids
                    
                    _logger.info('-------move_det************ = %s',move_det)
                    if move_det:
                        for search_result_ship in self.env['cost.build.up'].browse(move_det):
                            search_result_ship.write({"amount":amount})
                    break
    ...
    def _query_get(self, domain=None):
        ...
        if domain:
            ...
            _logger.warning(' =================tables==%s',tables)
            _logger.warning(' =================where_clause==%s',where_clause)
            _logger.warning(' =================where_clause_params==%s',where_clause_params)
            _logger.warning(' ================= query.get_sql()==%s', query.get_sql())
...
class CostBuilUp(models.Model):
    ...
    @api.model
    def create(self, vals):
        return super(CostBuilUp, self).create(vals)
