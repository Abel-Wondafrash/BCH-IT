...
String verifyOrderValidity (String code) {
  ...
  if (vTable == null) return "Could not verify order '" + SOV_PREFIX + code + "'";
  ...
  if (orderCode == null || !orderCode.equals (code))
    return "Order " + SOV_PREFIX + code + " DOES NOT EXIST!\nMake sure you are on the right DB.\n\nSelected DB: '" + DB_NAME + "'";

  processing.data.Table fsT = getFS (orderCode);
  // FS Fetching Validity Check
  if (fsT == null || !fsT.hasColumnTitles() || fsT.getRowCount () == 0) return "Failed to fetch FS for order '" + SOV_PREFIX + code + "'";
...
String entry (Order order) {
  ...
  if (!winWaitUntilExist (Windows.mainTitle, Timeouts.WIN_WAIT))
  ...
  if (!isWinForeground (Windows.mainTitle)) {
    ...
    if (!winWaitUntilForeground (Windows.mainTitle, Timeouts.WIN_WAIT))
      return "> Timeout waiting for window to be foreground";
  }

  //println ("Waiting for '" + Windows.cashSalesVoucher + "' window");
  if (!winWaitUntilContains (Windows.mainTitle, Windows.cashSalesVoucher, Timeouts.WIN_WAIT)) // CASH_SALES_VOUCHER_TITLE_IN_WINDOW_INDEX
  ...
  { /// SET CUSTOMER
    ...
    customerDD.click ();
    if (!waitWhilePixelColor (555, 255, -1, Timeouts.FIELD_WAIT)) return "> Timeout waiting for Customer entry field to appear";
    // Enter TIN
    ...
    if (!waitForPixelColor(555, 310, SELECTION_BLUE, Timeouts.FIELD_WAIT)) return "> Timeout waiting to select partner. TIN maybe mismatch or missing"; 
    // Select customer
    robot.ENTER ();
    if (!waitForFieldContent (tin, partnerTin, Timeouts.FIELD_WAIT))
  ...
  { /// Set ORIGIN (Remark)
    ...
    if (!waitForFieldContent (origin, orderName, Timeouts.FIELD_WAIT))
  ...
  { /// Set Order Lines
    for (OrderLine line : order.getLines ()) {
      ...
      articleDD.click();
      if (!waitWhilePixelColor (510, 325, -1, Timeouts.FIELD_WAIT)) return "> Timeout waiting for Article entry field to appear";
      //// Enter Product Code
      ...
      if (!waitForPixelColor(510, 378, SELECTION_BLUE, Timeouts.FIELD_WAIT)) return "> Timeout waiting to select Article. Article maybe mismatch or missing";
      // Select product
      robot.ENTER ();
      if (!waitForFieldContent (articleD, itemName, Timeouts.FIELD_WAIT))
      ...
      robot.ENTER ();
      if (!waitForFieldContent (quantity, itemQuantity, Timeouts.FIELD_WAIT)) return "> Timeout waiting to read Quantity content";
      ...
      if (!waitForFieldContent (price, itemPrice, Timeouts.FIELD_WAIT)) return "> Timeout waiting to read Price content";
  ...
  { /// Confirm Grand Total
    ...
    if (!waitForFieldContent (grandTotal, orderGrandTotal, Timeouts.FIELD_WAIT))
    ...
    if (delta > NOMINAL_DELTA) return "> Grand Total Delta is NOT nominal '" + delta +
  ...
  //if (!waitForFieldContent (fs, FS_NUMBER_SAMPLE, Timeouts.FIELD_WAIT))
  //    return "> Timeout waiting to read FS from CNET";
  //  String fsNumber = fs.getContent();
  //  if (fsNumber == null || fsNumber.isEmpty() || !isValidInteger(fsNumber))
  //    return "> Found invalid FS from CNET";

  if (!winWaitUntilExist (Windows.voucherSavedModal, Timeouts.WIN_WAIT*4)) return ("> Timeout waiting for " + Windows.voucherSavedModal + " Window to exist");
  closeWindowByTitle (Windows.voucherSavedModal);
  if (!winWaitIfExist (Windows.voucherSavedModal, Timeouts.WIN_WAIT*4)) return ("> Timeout waiting for " + Windows.voucherSavedModal + " to close");

  if (!winWaitUntilExist (Windows.printDialog, Timeouts.WIN_WAIT*4)) return ("> Timeout waiting for " + Windows.printDialog + " Window to exist");
  closeWindowByTitle (Windows.printDialog);
  if (!winWaitIfExist (Windows.printDialog, Timeouts.WIN_WAIT*4)) return ("> Timeout waiting for " + Windows.printDialog + " to close");
...
String isArticleEmpty () {
  if (!winWaitUntilExist (Windows.addLineItemModal, Timeouts.WIN_WAIT)) return "> Timeout waiting for " + Windows.addLineItemModal + " Window to exist";
  if (!fieldWaitUntilExist (Windows.addLineItemModal, "&OK", Timeouts.FIELD_WAIT)) return "> Timeout waiting for " + Windows.addLineItemModal + " Field to exist";
  robot.press (new Integer [] {KeyEvent.VK_ALT, KeyEvent.VK_O});

  if (!winWaitIfExist (Windows.addLineItemModal, Timeouts.WIN_WAIT)) return "> Timeout waiting for " + Windows.addLineItemModal + " to close";
  if (!winWaitUntilExist (Windows.mainTitle, Timeouts.WIN_WAIT)) return "> Timeout waiting for " + Windows.mainTitle + " to exist";

  return null;
}
String isOrderLineEmpty () {
  if (!winWaitUntilExist (Windows.removeLineItemModal, Timeouts.WIN_WAIT)) return "> Timeout waiting for " + Windows.removeLineItemModal + " Window to exist";
  if (hasFieldText (Windows.removeLineItemModal, new String [] {"&Yes", "&No"})) return "> Remove Added Lines First";

  if (!fieldWaitUntilExist (Windows.removeLineItemModal, "&OK", Timeouts.FIELD_WAIT)) return "> Timeout waiting for " + Windows.removeLineItemModal + " Field to exist";
  robot.press (new Integer [] {KeyEvent.VK_ALT, KeyEvent.VK_O});

  if (!winWaitIfExist (Windows.removeLineItemModal, Timeouts.WIN_WAIT)) return "> Timeout waiting for " + Windows.removeLineItemModal + " to close";
  if (!winWaitUntilExist (Windows.mainTitle, Timeouts.WIN_WAIT)) return "> Timeout waiting for " + Windows.mainTitle + " to exist";
