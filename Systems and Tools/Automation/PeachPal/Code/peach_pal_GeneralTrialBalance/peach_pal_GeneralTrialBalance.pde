RobotTools robot;
static final String WIN_VBB = "Chart of Accounts Beginning Balances";
...
void setup () {
  ...
  String path = dataPath ("") + "/cb_trial_bal-aug12.csv";
  int delayAfterSetting = 400;

  Table table = loadTable (path, "header");
  //String codes [] = table.getStringColumn ("partner_code");
  String labels [] = table.getStringColumn ("label");
  String vals [] = table.getStringColumn ("value");

  String classes [] = {"ABD_Dollars1"};
  for (int i = 0; i < labels.length; i ++) { // names.length
    println (nf (i + 1, 3), "of", nf (labels.length, 3), nfc ((i + 1.0)/labels.length, 2), "%\t", labels [i], vals [i]);
    String values [] = {vals [i]};
    if (float (vals [i]) == 0) continue;
