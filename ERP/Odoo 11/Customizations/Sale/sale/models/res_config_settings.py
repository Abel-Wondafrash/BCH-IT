...
class ResConfigSettings(models.TransientModel):
    ...
    def _get_loc(self):
        search_results= self.env['res.config.settings'].search([]).ids
        
        if search_results:
            if self.env['res.config.settings'].search([])[-1].xml_drop_location:
                _logger.info('-------!!!!!!!!xml_drop_location=%s',self.env['res.config.settings'].search([])[-1].xml_drop_location)
                return str(self.env['res.config.settings'].search([])[-1].xml_drop_location)
    ...
    xml_drop_location = fields.Char("XML drop location",default=_get_loc)
    xml_drop_location_customer_sms = fields.Char("XML drop location For customer SMS",default=_get_loc)
    ...
    def get_values(self):
        ...
        res.update(
            ...
            xml_drop_location=ICPSudo.get_param('sale.xml_drop_location'),
    ...
    def set_values(self):
        ...
        ICPSudo.set_param('sale.xml_drop_location', self.xml_drop_location)
