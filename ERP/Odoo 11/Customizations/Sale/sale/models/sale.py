...
class SaleOrder(models.Model):
    ...
    def _computeBalance(self):
        for order in self:
            clause_final = [('id', '=', order.partner_id.id)]
            
            search_results= self.env['res.partner'].search(clause_final).ids
            if search_results:
                for search_result in self.env['res.partner'].browse(search_results):
                    _logger.info('-**------!!!!!!!!current_balance=%s',search_result.current_balance)
                    # self.current_balance=search_result.current_balance

                    order.update({
                        'current_balance':search_result.current_balance,
                    })
    ...
    def _amount_all(self):
        ...
        for order in self:
            ...
            new_total=amount_untaxed + amount_tax
            order.update({
            ...
            if order.current_balance< new_total-40:
                raise UserError(_('This customer have no enough balance.'))
    ...
    validity_date = fields.Date(
        string='Expiration Date', readonly=True, copy=False,
    ...
    pricelist_id = fields.Many2one('product.pricelist', string='Pricelist', required=True, readonly=True, states={'draft': [('readonly', True)], 'sent': [('readonly', True)]}, help="Pricelist for current sales order.")
    ...
    # current_balance = fields.Monetary(string='Current Balance', store=True, readonly=True)
    current_balance = fields.Monetary(string='Current Balance', store=True, readonly=True, compute='_computeBalance')
    ...
    sales_type = fields.Selection([
        ...
        string='Sales Type', store=True, default='pre-payment',
        readonly=True)

    # @api.multi
    # @api.onchange('partner_id')
    # def onchange_partner(self):
    #     clause_final = [('id', '=', self.partner_id.id)]
    ...
    @api.multi
    def action_confirm_create_xml(self):
        for record in self: record.create_xml()

    @api.multi
    def create_xml(self):
        total=0
        
        sales_config = self.env['res.config.settings'].search([], order='id desc', limit=1)
        # xml_file_location = sales_config.xml_drop_location if sales_config else None
        # if not xml_file_location:
        #     raise UserError(_('XML path not found. Please insert xml file path on sales configuration.'))
        
        OUTPUT_DIR = r"C:\Users\Loj\XMLs\Vouchers"
        name = self.name + '-'
        now = datetime.today().strftime('%y%m%d%H%M%S')
        _logger.info('-------!!!!!!!!Start date =%s', now)
        
        vouchers_path = os.path.normpath(OUTPUT_DIR)
        try:
            if not os.path.exists(vouchers_path):
                os.makedirs(vouchers_path)
        except OSError as e:
            _logger.error(f"Error creating directory {vouchers_path}: {e}")
            raise UserError(_('Could not create the Vouchers directory. Please check your permissions.'))

        OUTPUT_DIR = os.path.join(vouchers_path, f"{name}{now}.xml")
        _logger.info('-------!!!!!!!!Start OUTPUT_DIR- =%s', OUTPUT_DIR)
        
        root = minidom.Document()

        xml = root.createElement('VoucherXml')
        root.appendChild(xml)

        second_root=root.createElement('Voucher')
        xml.appendChild(second_root)

        # Determine how many copies to create
        copies = self.env.context.get('copies', 1)  # Default to 1 if not specified
        copy_type = self.env.context.get('copy_type', 'B') # Default to 'B' (Blank) if not specified

        childOfproduct = root.createElement('copies')
        childOfproduct.appendChild(root.createTextNode(str(copies)))
        ...
        childOfproduct.appendChild(root.createTextNode(self.name))
        second_root.appendChild(childOfproduct)

        date = datetime.strptime(str(self.date_order),'%Y-%m-%d %H:%M:%S')
        _logger.info('-------!!!!!!!!Start date- =%s',date)
        dateFormated=date.strftime("%Y-%m-%dT%H:%M:%S")
        _logger.info('-------!!!!!!!!Start dateFormated- =%s',dateFormated)
        childOfproduct = root.createElement('date')
        childOfproduct.appendChild(root.createTextNode(str(dateFormated)))
        second_root.appendChild(childOfproduct)

        # Your Reference
        if self.client_order_ref:
            childOfproduct = root.createElement('reference')
            childOfproduct.appendChild(root.createTextNode(self.client_order_ref))
            second_root.appendChild(childOfproduct)

        # Date Ordered
        if self.confirmation_date and self.state not in ['sale', 'sent', 'done']:
            childOfproduct = root.createElement('dateOrdered')
            childOfproduct.appendChild(root.createTextNode(str(self.confirmation_date)))
            second_root.appendChild(childOfproduct)

        # Quotation Date
        childOfproduct = root.createElement('dateQuoted')
        childOfproduct.appendChild(root.createTextNode(str(self.date_order)))
        second_root.appendChild(childOfproduct)

        # Salesperson
        if self.user_id:
            childOfproduct = root.createElement('salesperson')
            childOfproduct.appendChild(root.createTextNode(self.user_id.name or ""))
            second_root.appendChild(childOfproduct)

        # Plate Number
        if self.plate_no:
            childOfproduct = root.createElement('plateNumber')
            childOfproduct.appendChild(root.createTextNode(self.plate_no or ""))
            second_root.appendChild(childOfproduct)

        # Stock (Warehouse Name)
        if self.warehouse_id:
            childOfproduct = root.createElement('stock')
            childOfproduct.appendChild(root.createTextNode(self.warehouse_id.name or ""))
            second_root.appendChild(childOfproduct)

        # Payment Terms
        if self.payment_term_id:
            childOfproduct = root.createElement('paymentTerm')
            childOfproduct.appendChild(root.createTextNode(self.payment_term_id.name or ""))
            second_root.appendChild(childOfproduct)

        # Customer Details
        third_root=root.createElement('customer')
        second_root.appendChild(third_root)

        # Customer Code
        childOfproduct = root.createElement('code')
        childOfproduct.appendChild(root.createTextNode(str(self.partner_id.partner_code)))
        third_root.appendChild(childOfproduct)

        # Customer Name
        childOfproduct = root.createElement('name')
        childOfproduct.appendChild(root.createTextNode(str(self.partner_id.name)))
        third_root.appendChild(childOfproduct)

        # Customer Location
        location_name = self.location.name if self.location else '-'
        childOfproduct = root.createElement('location')
        childOfproduct.appendChild(root.createTextNode(location_name))
        third_root.appendChild(childOfproduct)

        # Customer Address
        childOfproduct = root.createElement('address')
        childOfproduct.appendChild(root.createTextNode(str(self.partner_id.contact_address)))
        third_root.appendChild(childOfproduct)

        # Customer TIN
        childOfproduct = root.createElement('TIN')
        childOfproduct.appendChild(root.createTextNode(str(self.partner_id.vat)))
        third_root.appendChild(childOfproduct)
		
        # Voucher Details
        sub_total=0.0
        clause_final = [('order_id', '=', self.id)]
        search_results= self.env['sale.order.line'].search(clause_final).ids
        
        if search_results:
            for search_result_order_line in self.env['sale.order.line'].browse(search_results):
                sub_total=sub_total+ search_result_order_line.price_subtotal

                fourth_root=root.createElement('LineItem')
                xml.appendChild(fourth_root)

                # Get UOM Sale ID or default to 'PACK'
                uom_sale_id = search_result_order_line.product_id.uom_sale_id
                uom_name = uom_sale_id.name if uom_sale_id else 'PACK'

                # Ensure uom_name is not empty, setting to 'PACK' if it is
                uom_name = uom_name if uom_name else 'PACK'

                # Add UOM to XML
                childOfuom = root.createElement('uom')
                childOfuom.appendChild(root.createTextNode(str(uom_name)))
                fourth_root.appendChild(childOfuom)

                childOfproduct = root.createElement('pack')
                childOfproduct.appendChild(root.createTextNode(str(search_result_order_line.product_pack)))
                fourth_root.appendChild(childOfproduct)

                childOfproduct = root.createElement('quantity')
                childOfproduct.appendChild(root.createTextNode(str(search_result_order_line.product_uom_qty)))
                fourth_root.appendChild(childOfproduct)

                childOfproduct = root.createElement('unitPrice')
                childOfproduct.appendChild(root.createTextNode(str(search_result_order_line.price_unit)))
                fourth_root.appendChild(childOfproduct)

                childOfproduct = root.createElement('subtotal')
                childOfproduct.appendChild(root.createTextNode(str(search_result_order_line.price_subtotal)))
                fourth_root.appendChild(childOfproduct)

                # Tax Type
                tax_names = ', '.join([tax.description or tax.name for tax in search_result_order_line.tax_id])
                childOfproduct = root.createElement('taxType')
                childOfproduct.appendChild(root.createTextNode(tax_names or 'NO_TAX'))
                fourth_root.appendChild(childOfproduct)

                childOfproduct = root.createElement('taxAmount')
                childOfproduct.appendChild(root.createTextNode(str(search_result_order_line.price_tax)))
                fourth_root.appendChild(childOfproduct)

                childOfproduct = root.createElement('totalAmount')
                childOfproduct.appendChild(root.createTextNode(str(search_result_order_line.price_total)))
                fourth_root.appendChild(childOfproduct)

                # Item Details
                fourth_root_child = root.createElement('item')
                fourth_root.appendChild(fourth_root_child)
                ...
                childOfproduct = root.createElement('name')
                childOfproduct.appendChild(root.createTextNode(str(product.name or '')))
                fourth_root_child.appendChild(childOfproduct)

                childOfproduct = root.createElement('category')
                childOfproduct.appendChild(root.createTextNode(str(product.categ_id.name or '')))
                fourth_root_child.appendChild(childOfproduct)


                additional_item_details=root.createElement('lineItemValues')
                fourth_root.appendChild(additional_item_details)

                childOfproduct = root.createElement('discount')
                childOfproduct.appendChild(root.createTextNode('0.00'))
                additional_item_details.appendChild(childOfproduct)

                childOfproduct = root.createElement('additionalCharge')
                childOfproduct.appendChild(root.createTextNode('0.00'))
                additional_item_details.appendChild(childOfproduct)

        fivth_root=root.createElement('voucherValues')
        xml.appendChild(fivth_root)

        childOfproduct = root.createElement('discount')
        childOfproduct.appendChild(root.createTextNode('0.00'))
        fivth_root.appendChild(childOfproduct)
        ...
        childOfproduct = root.createElement('subTotal')
        childOfproduct.appendChild(root.createTextNode(str(sub_total)))
        fivth_root.appendChild(childOfproduct)

        # Get the tax amount by group
        tax_groups = self._get_tax_amount_by_group()

        # Iterate over the tax groups to extract the total tax amount
        tax_total = 0.0
        for group in tax_groups:
            tax_total += group[1]  # group[1] holds the tax amount for this group

        childOfproduct = root.createElement('taxTotal')
        childOfproduct.appendChild(root.createTextNode(str(tax_total)))
        fivth_root.appendChild(childOfproduct)

        childOfproduct = root.createElement('grandTotal')
        childOfproduct.appendChild(root.createTextNode(str(self.amount_total)))
        fivth_root.appendChild(childOfproduct)

        sixth_root=root.createElement('Activity')
        xml.appendChild(sixth_root)

        childOfproduct = root.createElement('deviceName')
        childOfproduct.appendChild(root.createTextNode(str(socket.gethostname())))
        sixth_root.appendChild(childOfproduct)

        curent_user = self.env['res.users'].search([('id', '=', self.create_uid.id)], limit=1)
        sixth_root.appendChild(childOfproduct)
        sixth_root_child=root.createElement('user')
        sixth_root.appendChild(sixth_root_child)

        _logger.info('-------!!!!!!!!self.create_uid date =%s',self.create_uid)
        resource = self.env['resource.resource'].search([('user_id','=',self.create_uid.id)])
        employee = self.env['hr.employee'].search([('resource_id','=',resource.id)])
        _logger.info('-------!!!!!!!!self.create_uid employee =%s',employee)
        _logger.info('-------!!!!!!!!self.create_uid employee.identification_i =%s',employee.identification_id)

        childOfproduct = root.createElement('fullName')
        childOfproduct.appendChild(root.createTextNode(str(self.env.user.name)))
        sixth_root_child.appendChild(childOfproduct)
        
        childOfproduct = root.createElement('userName')
        childOfproduct.appendChild(root.createTextNode(str(curent_user.partner_code)))
        sixth_root_child.appendChild(childOfproduct)
        ...
        _logger.info('-------!!!!!!!!Start OUTPUT_DIR0000****--*** =%s',OUTPUT_DIR)
    
    def xml_log(self, log_content, file_prefix="log"):
        # Define output directory
        OUTPUT_DIR = "F:\\Loj\\XMLs\\Vouchers"
        vouchers_path = os.path.normpath(OUTPUT_DIR)  # Normalize path

        try:
            # Ensure the directory exists
            if not os.path.exists(vouchers_path):
                os.makedirs(vouchers_path)
            _logger.info(f"Directory {vouchers_path} exists or was created successfully.")

            # Generate a unique file name
            now = fields.Datetime.now().strftime('%Y%m%d_%H%M%S')
            file_name = f"{file_prefix}_{now}.xml"
            file_path = os.path.join(vouchers_path, file_name)
            ...
            _logger.info(f"XML log created successfully at: {file_path}")
        except Exception as e:
            _logger.error(f"Error creating XML log: {e}")
            raise UserError(_('An error occurred while creating the XML log. Please check the server logs for more details.'))
    ...
    def delete(self):
        for order in self:
            if order.state not in ('done', 'sale', 'sent',):
                self._cr.execute("DELETE FROM sale_order WHERE id=%s", [order.id])
    ...
    def _onchange_pricelist_update_order_lines(self):
        for order in self:
            if not order.pricelist_id:
                continue  # Skip if no pricelist
            for line in order.order_line:
                if not line.product_id:
                    continue  # Skip if no product
                try:
                    # Get the price using the actual pricelist
    ...
    def create(self, vals):
        # Log immediately as the method is entered
        _logger.info("Entering SaleOrder.create method")
        
        # _logger.info('-------!!!!!!!!amount_total=%s',self.amount_total)
        # # _logger.info('-------!!!!!!!!valamount_total=%s',vals.get('amount_total') )
        # _logger.info('-------!!!!!!!!current_balance=%s',self.current_balance)
        # # _logger.info('-------!!!!!!!!valcurrent_balance=%s',vals.get('current_balance'))
        # for order in self:
        #     _logger.info('-------!!!!!!!!amount_total=%s',order.amount_total)
        #     _logger.info('-------!!!!!!!!current_balance=%s',order.current_balance)
        #     if order.current_balance < order.amount_total:
        #         raise UserError(_('This customer have no enouh balance.'))
        #     else:
        ...
        #         vals['payment_term_id'] = immediate_payment_term.id
    ...
    def _prepare_invoice(self):
        ...
        # change account id here based on sales type
        
        if self.sales_type =="pre-payment":
            
            _logger.info('-------!!!!!!!!amount_total=%s',self.amount_total)
            total_sales_amount=self.amount_total
            clause_final = [('id', '=', self.partner_invoice_id.id)]
            total_balance=0
            search_results= self.env['res.partner'].search(clause_final).ids
            if search_results:
                for search_result in self.env['res.partner'].browse(search_results):
                    _logger.info('-------!!!!!!!!current_balance=%s',search_result.current_balance)
                    total_balance=total_balance + search_result.current_balance

            diff = total_balance - total_sales_amount
            ...
            if diff >= 0:
                ...
                'origin': self.name,
                'type': 'out_invoice',
                'account_id': self.partner_invoice_id.pre_payment_account_id.id,
    ...
    def action_cancel(self):
        clause_final_sales = [('sale_id', '=', self.id)]
        search_result_picks= self.env['stock.picking'].search(clause_final_sales).ids
        
        if search_result_picks:
            for search_result_pick in self.env['stock.picking'].browse(search_result_picks):
                search_result_pick.write({'state': 'cancel'})
    ...
    def _action_confirm(self):
        ...
        if order.sales_type:
            ...
            raise UserError(_('Sales type must be filled.'))
    ...
    @api.multi
    def marketing_action_confirm(self):

        self.state='confirm'
        self.message_post("Marketing Confirmed")
    @api.multi
...
class SaleOrderLine(models.Model):
    ...
    @api.depends('product_pack')
    def _compute_pack(self):
        for line in self:
            if line.product_id.product_pack_qty:
                _logger.info('-------.product_pack_qty=%s',line.product_id.product_pack_qty)
        
                line.product_uom_qty =line.product_pack *line.product_id.product_pack_qty
    ...
    product_pack = fields.Float(string='Pack', store=True)
    ...
    tax_id = fields.Many2many('account.tax', string='Taxes', domain=['|', ('active', '=', False), ('active', '=', True)], readonly=True)
    ...
    # avalible_qty= fields.Float(string='Avalible Quantity')
    product_uom_qty = fields.Float(string='Quantity',  
    digits=dp.get_precision('Product Unit of Measure'), compute='_compute_pack',store=True, default=1.0,readonly=True,)
    # product_pack = fields.Float(string='Pack')
    ...
    # def batch_no_change(self):
    #     if self.batch_no:
    #         clause_final = ['&',('batch_no', '=', self.batch_no),('state', '!=','solved')]
    #         search_results= self.env['quality.alert'].search(clause_final).ids
    #         if search_results:
    #             self.batch_no=False
    #             raise UserError(_('This batch number has failed test result. Please select other batch number!'))
    ...
    def product_id_change(self):
        ...
        _logger.info('-------!!!!!!!!self.product_id.product_pack_qty=%s',self.product_id.product_pack_qty)
        
        vals = {}
        # if self.product_id.product_pack_qty:
        #     vals['product_pack']=vals.get('product_uom_qty') /self.product_id.product_pack_qty
        # clause_final = ['&',('product_id', '=', self.product_id.id),('location_id','=',12)]
        # search_results= self.env['stock.quant'].search(clause_final).ids
        # total_qty=0
        # if search_results:
        #     for search_result_product in self.env['stock.quant'].browse(search_results):
        #         Qty=search_result_product.quantity
        #         total_qty=total_qty + Qty
        #         _logger.info('-------!!!!!!!!total_qty=%s',total_qty)
        # self.avalible_qty=total_qty
        ...
        # Compute taxes considering fiscal position
        fpos = self.order_id.fiscal_position_id or self.order_id.partner_id.property_account_position_id
        taxes = product.supplier_taxes_id.filtered(lambda r: not self.company_id or r.company_id == self.company_id)
        # Set the taxes for this sales order line. If a fiscal position is defined, it maps the product's vendor taxes 
        # according to the fiscal position rules for the shipping partner; otherwise, it uses the product's vendor taxes directly.
        vals['tax_id'] = [(6, 0, fpos.map_tax(taxes, product, self.order_id.partner_shipping_id).ids if fpos else taxes.ids)]
        ...
        # if name =='%1':
        #     vals['product_pack']=vals.get('product_uom_qty') /6
        # elif name =='%2':
        #     vals['product_pack']=vals.get('product_uom_qty') /6
        # elif name =='%0.5':
        #     vals['product_pack']=vals.get('product_uom_qty') /12
        # elif name =='%0.6':
        #     vals['product_pack']=vals.get('product_uom_qty') /12
    ...
    def product_uom_change(self):
        if not self.product_uom or not self.product_id:
            ...
            return
        # if self.product_id.product_pack_qty:
        #     self.product_pack=self.product_uom_qty /self.product_id.product_pack_qty
        if self.order_id.pricelist_id and self.order_id.partner_id:
            product = self.product_id.with_context(
                lang=self.order_id.partner_id.lang,
                partner=self.order_id.partner_id.id,
