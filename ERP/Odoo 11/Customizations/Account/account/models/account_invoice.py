...
class AccountPrintinvoice(models.Model):
    _name = "account.printinvoice"
    print_date = fields.Datetime('Print Date')
    account_invoice = fields.Many2one('account.invoice', string='account_invoice')
...
class AccountInvoice(models.Model):
    ...
    name = fields.Char(string='Reference/Description', index=True, copy=False, help='The name that will be used on account move lines')
    ...
    invoice_printed = fields.Boolean(
        'Invoice Printed',default=False)
    ...
    fs_no = fields.Integer(string='Journal Entry Name',default=0)
    ...
    def _write(self, vals):
        res = super(AccountInvoice, self)._write(vals)
        for invoice in self:
            if invoice.sales_type != "pre-payment":
                if invoice.reconciled and invoice.state == 'open':
                    invoice.action_invoice_paid()
                elif not invoice.reconciled and invoice.state == 'paid':
    ...
    def create_xml(self):
        ...
        if not user.partner_id.partner_code:
            raise UserError(_('User not allowed to print receipt'))
        ...
        # Construct the dynamic folder path: Invoices/2025/May/MAY_16_2025
        ...
        try:
            ...
            _logger.error(f"Error creating directory {invoices_path}: {e}")
            raise UserError(_('Could not create the Invoices directory. Please check your permissions.'))

        # Construct the file name: INV-<origin or name>-<timestamp>.xml
        name = 'INV-' + (self.origin or self.name) + '-'
        OUTPUT_DIR = os.path.join(invoices_path, f"{name}{timestamp}.xml")
        _logger.info('-------!!!!!!!!Output file path = %s', OUTPUT_DIR)
        ...
        # Generate FS number
        last_invoice = self.env['account.invoice'].search([], order='fs_no desc', limit=1)
        fs_no = last_invoice.fs_no + 1 if last_invoice else 1
        self.write({'fs_no': fs_no, 'cash_invoice_printed': True})
        formated_number = "{:08d}".format(fs_no)
        formated_fs = 'FS No.' + formated_number

        childOfproduct = root.createElement('code')
        childOfproduct.appendChild(root.createTextNode(formated_fs))
        second_root.appendChild(childOfproduct)

        if self.origin:
            childOfsource = root.createElement('source')
            childOfsource.appendChild(root.createTextNode(self.origin))
            second_root.appendChild(childOfsource)

        childOfproduct = root.createElement('voucherDefnition')
        childOfproduct.appendChild(root.createTextNode('106'))
        second_root.appendChild(childOfproduct)

        dateFormated = datetime.now().strftime("%Y-%m-%dT%H:%M:%S")
        _logger.info('-------!!!!!!!!Formatted date = %s', dateFormated)
        childOfproduct = root.createElement('date')
        childOfproduct.appendChild(root.createTextNode(dateFormated))
        second_root.appendChild(childOfproduct)

        # Calculate grand total
        current_invoice_total = 0
        for line in self.invoice_line_ids:
            current_tax_amount = line.price_subtotal * 0.15
            current_invoice_total += line.price_subtotal + current_tax_amount

        childOfproduct = root.createElement('grandTotal')
        childOfproduct.appendChild(root.createTextNode(str(current_invoice_total)))
        second_root.appendChild(childOfproduct)
        ...
        childOfproduct = root.createElement('code')
        childOfproduct.appendChild(root.createTextNode(str(self.partner_id.partner_code or '')))
        third_root.appendChild(childOfproduct)

        childOfproduct = root.createElement('name')
        childOfproduct.appendChild(root.createTextNode(str(self.partner_id.name or '')))
        third_root.appendChild(childOfproduct)
        ...
        childOfproduct.appendChild(root.createTextNode(str(self.partner_id.vat or '')))
        third_root.appendChild(childOfproduct)

        # Line items
        sub_total = sum(line.price_subtotal for line in self.invoice_line_ids)
        for line in self.invoice_line_ids:
            fourth_root = root.createElement('LineItem')
            xml.appendChild(fourth_root)

            childOfproduct = root.createElement('unitAmount')
            childOfproduct.appendChild(root.createTextNode(
                str(line.price_unit * line.product_id.product_pack_qty)))
            fourth_root.appendChild(childOfproduct)

            childOfproduct = root.createElement('quantity')
            childOfproduct.appendChild(root.createTextNode(
                str(line.quantity / line.product_id.product_pack_qty)))
            fourth_root.appendChild(childOfproduct)

            childOfproduct = root.createElement('totalAmount')
            childOfproduct.appendChild(root.createTextNode(str(line.price_subtotal)))
            fourth_root.appendChild(childOfproduct)

            childOfproduct = root.createElement('taxType')
            childOfproduct.appendChild(root.createTextNode('1'))
            fourth_root.appendChild(childOfproduct)

            childOfproduct = root.createElement('taxAmount')
            tax_amount = (15 / 100) * line.price_subtotal
            childOfproduct.appendChild(root.createTextNode(str(tax_amount)))
            fourth_root.appendChild(childOfproduct)

            childOfproduct = root.createElement('totalWithTaxes')
            childOfproduct.appendChild(root.createTextNode(str(line.price_total)))
            fourth_root.appendChild(childOfproduct)

            # Product details
            pro_code = ''
            pro_category = line.product_id.categ_id.name or ''
            pro_name = line.product_id.name or ''
            product_descriptor = self.env['product.descriptor'].search([
                ('product_temp', '=', line.product_id.product_tmpl_id.id),
                ('warehouse_id', '=', sale_order.warehouse_id.id)
            ], limit=1)
            if product_descriptor:
                pro_code = product_descriptor.productWarehouseId or ''

            fourth_root_child = root.createElement('item')
            fourth_root.appendChild(fourth_root_child)

            childOfproduct = root.createElement('code')
            childOfproduct.appendChild(root.createTextNode(pro_code))
            fourth_root_child.appendChild(childOfproduct)

            childOfproduct = root.createElement('name')
            childOfproduct.appendChild(root.createTextNode(pro_name))
            fourth_root_child.appendChild(childOfproduct)

            childOfproduct = root.createElement('category')
            childOfproduct.appendChild(root.createTextNode(pro_category))
            fourth_root_child.appendChild(childOfproduct)

            fourth_root_child_two = root.createElement('lineItemValues')
            fourth_root.appendChild(fourth_root_child_two)

            childOfproduct = root.createElement('discount')
            childOfproduct.appendChild(root.createTextNode('0.00'))
            fourth_root_child_two.appendChild(childOfproduct)
        ...
        # Voucher values
        fifth_root = root.createElement('voucherValues')
        xml.appendChild(fifth_root)

        childOfproduct = root.createElement('discount')
        childOfproduct.appendChild(root.createTextNode('0.00'))
        fifth_root.appendChild(childOfproduct)
        ...
        childOfproduct.appendChild(root.createTextNode(socket.gethostname()))
        sixth_root.appendChild(childOfproduct)

        sixth_root_child = root.createElement('user')
        sixth_root.appendChild(sixth_root_child)

        resource = self.env['resource.resource'].search([('user_id', '=', self.create_uid.id)], limit=1)
        employee = self.env['hr.employee'].search([('resource_id', '=', resource.id)], limit=1)

        childOfproduct = root.createElement('employeeCode')
        childOfproduct.appendChild(root.createTextNode(str(employee.identification_id or '')))
        sixth_root_child.appendChild(childOfproduct)

        childOfproduct = root.createElement('fullName')
        childOfproduct.appendChild(root.createTextNode(str(employee.name or '')))
        sixth_root_child.appendChild(childOfproduct)

        childOfproduct = root.createElement('userName')
        childOfproduct.appendChild(root.createTextNode(str(self.create_uid.partner_code or '')))
        sixth_root_child.appendChild(childOfproduct)
        ...
        try:
            ...
            _logger.info('-------!!!!!!!!XML file created at %s', OUTPUT_DIR)
        except OSError as e:
            _logger.error(f"Error writing XML file {OUTPUT_DIR}: {e}")
            raise UserError(_('Could not write the XML file. Please check your permissions.'))

        # Update invoice and create print record
        printer_info = {
            'print_date': datetime.now(),
            'account_invoice': self.id
        }
        self.write({'invoice_printed': True})
        self.env['account.printinvoice'].create(printer_info)
    ...
    def action_invoice_open(self):
        ...
        _logger.warning(' self.sales_type=%s ',self.sales_type)


        if self.sales_type == 'pre-payment':
            _logger.warning(' ===================pre paymeny')
            # create jornal enty with given account number
            to_open_invoices.action_move_create()
            self.write({'state': 'paid'})
        else:
            _logger.warning(' ===================under else')
            to_open_invoices.action_move_create()
            return to_open_invoices.invoice_validate()
        sale_order = self.env['sale.order'].search([('name', '=', self.origin)], order='id desc', limit=1)
        clause_final_sales = [('sale_id', '=', sale_order.id)]
        search_result_picks = self.env['stock.picking'].search(clause_final_sales, limit=1)
        for search_resut_pick in search_result_picks.move_lines:
            search_resut_pick.write({'quantity_done':search_resut_pick.product_uom_qty})
            # raise UserError(str(search_resut_pick))
        search_result_picks.button_validate()   
        self.create_xml()
    ...
    def action_move_create(self):
        ...
        for inv in self:
            ...
            invice_ref=""
            if inv.reference:
                invice_ref=inv.reference
            else:

                # _logger.info('-------!!!!!!!!journal_sequence=%s',self._get_sequence_number_next())

                invice_ref="INV/2021/"+str(self.id)+"/"+date
            ...
            _logger.info('-------!!!!!!!!ctx_nolang=%s',ctx_nolang)
...
class AccountInvoiceAllow(models.Model):
    _name = "account.invoice.allow"
    _description = "Print Invoice Confirm"

    def print_invoice(self):
        id = self._context.get('active_id')
        clause_final = [('id', '=', id)]
        self = self.env['account.invoice'].search(clause_final,limit=1)
        total = 0

        clause_final = [('name', '=', self.origin)]
        sale_order = self.env['sale.order'].search([('name', '=', self.origin)], order='id desc', limit=1)
        OUTPUT_DIR = ""
        # if sales_configs:
        #     for sales_config in self.env['res.config.settings'].browse(sales_configs):
        #         xml_file_location=sales_config.xml_drop_location
        ...
        file_name = '\\' + str(name) + str(now) + str(".xml")
        ...
        childOfproduct.appendChild(root.createTextNode('106'))
        second_root.appendChild(childOfproduct)
        # date = datetime.strptime(str(self.date_invoice), '%Y-%m-%d %H:%M:%S')
        _logger.info('-------!!!!!!!!Start date- =%s', self.date_invoice)
        ...
        third_root = root.createElement('customer')
        # third_root.setAttribute('id', 'test-id')
        second_root.appendChild(third_root)
        ...
        sub_total = 0.0
        clause_final = [('invoice_id', '=', self.id)]
        search_results = self.env['account.invoice.line'].search(clause_final).ids

        if search_results:
            for search_result_order_line in self.env['account.invoice.line'].browse(search_results):
                ...
                childOfproduct = root.createElement('quantity')
                childOfproduct.appendChild(root.createTextNode(str(search_result_order_line.quantity)))
                fourth_root.appendChild(childOfproduct)

                childOfproduct = root.createElement('totalAmount')
                childOfproduct.appendChild(root.createTextNode(str(search_result_order_line.price_subtotal)))
                fourth_root.appendChild(childOfproduct)
                current_tax_type_id = 1
                if search_result_order_line.invoice_line_tax_ids.id != 1:
                    current_tax_type_id = 4
                childOfproduct = root.createElement('taxType')
                childOfproduct.appendChild(root.createTextNode(str(current_tax_type_id)))
                fourth_root.appendChild(childOfproduct)

                childOfproduct = root.createElement('taxAmount')
                tax_amount = (
                                         search_result_order_line.invoice_line_tax_ids.amount / 100) * search_result_order_line.price_subtotal
                childOfproduct.appendChild(root.createTextNode(str(tax_amount)))
                fourth_root.appendChild(childOfproduct)
                pro_code = ''
                pro_category = ''
                pro_name = ''
                clause_final_product = [('id', '=', search_result_order_line.product_id.id)]
                search_result_products = self.env['product.template'].search(clause_final_product).ids

                if search_result_products:
                    for search_result_product in self.env['product.template'].browse(search_result_products):
                        pro_category = search_result_product.categ_id.name
                        pro_name = search_result_product.name
                        code_search_clause = [('product_temp', '=', search_result_order_line.product_id.id),
                                              ('warehouse_id', '=', sale_order.warehouse_id.id)]
                        search_prcode_result_products = self.env['product.descriptor'].search(code_search_clause).ids
                        for search_prcode_result_product in self.env['product.descriptor'].browse(
                                search_prcode_result_products):
                            pro_code = search_prcode_result_product.productWarehouseId
                ...
                childOfproduct = root.createElement('code')
                childOfproduct.appendChild(root.createTextNode(str(pro_code)))
                fourth_root_child.appendChild(childOfproduct)

                childOfproduct = root.createElement('name')
                childOfproduct.appendChild(root.createTextNode(str(pro_name)))
                fourth_root_child.appendChild(childOfproduct)

                childOfproduct = root.createElement('category')
                childOfproduct.appendChild(root.createTextNode(str(pro_category)))
                fourth_root_child.appendChild(childOfproduct)

                ourth_root_child_two = root.createElement('lineItemValues')
                # third_root.setAttribute('id', 'test-id')
                fourth_root.appendChild(ourth_root_child_two)

                childOfproduct = root.createElement('discount')
                childOfproduct.appendChild(root.createTextNode('0.00'))
                ourth_root_child_two.appendChild(childOfproduct)
        ...
        childOfproduct = root.createElement('employeeCode')
        childOfproduct.appendChild(root.createTextNode(str(employee.identification_id)))
        sixth_root_child.appendChild(childOfproduct)

        childOfproduct = root.createElement('fullName')
        childOfproduct.appendChild(root.createTextNode(str(employee.name)))
        sixth_root_child.appendChild(childOfproduct)
