...
class ResPartner(models.Model):
    ...
    @api.multi
    def _get_total_deposit(self):
        for partner in self:
            _logger.info('-------##########partner.customer************ = %s',partner.customer)
            _logger.info('-------##########partner.supplier************ = %s',partner.supplier)
            if partner.customer:
                inital_balance=partner.inital_balance
                total_paid=0.0
                total_reverse=0.0
                _logger.info('-------partner.id************ = %s',partner.id)
                move_det = self.env['account.move'].search(['&',('ref','like','BKDP%'),('partner', '=', partner.id),('state', '=', 'posted')]).ids
                    
                _logger.info('-------move_det************ = %s',move_det)
                if move_det:
                    for search_result_ship in self.env['account.move'].browse(move_det):
                        _logger.info('-----search_result_ship.amount_val************ = %s',search_result_ship.amount_val)
                        total_paid=total_paid + search_result_ship.amount_val
                        _logger.info('------before -total_paid************ = %s',total_paid)
                        reverse_name="reversal of: "+search_result_ship.name
                        move_det_reverse = self.env['account.move'].search(['&',('ref','=',reverse_name),('partner', '=', partner.id),('state', '=', 'posted')]).ids
                            
                        _logger.info('-------move_det_reverse************ = %s',move_det_reverse)
                        if move_det_reverse:
                            for search_result_ship_reverse in self.env['account.move'].browse(move_det_reverse):
                                _logger.info('-----search_result_ship_reverse.amount_val************ = %s',search_result_ship_reverse.amount_val)
                                total_reverse=total_reverse + search_result_ship_reverse.amount_val
                total_paid = total_paid - total_reverse
                partner.total_deposit=0.0

    @api.multi
    def _get_net_cash(self):
        for current_partener in self:
            total_uninvoiced=0
            not_invoiced_sales= self.env['sale.order'].search([('invoice_status', '!=', 'invoiced'),('partner_id', '=', current_partener.id)]) 
            for not_invoiced_sale in not_invoiced_sales:
                total_uninvoiced=total_uninvoiced+not_invoiced_sale.amount_total

            current_partener.net_cash=current_partener.current_balance-total_uninvoiced    


           
    @api.multi
    def _get_curent_balance(self):

  
        # partners = self.env['res.partner'].search([('supplier','=',True)]).ids
        # if partners:
        #     _logger.info('-------partners************ = %s',partners)
        #     for partner in self.env['res.partner'].browse(partners):
        #         partner.write({"property_account_payable_id":2679,"property_account_receivable_id":2679,"pre_payment_account_id":2679})
        ...
        for partner in self:
            ...
            if partner.customer:
                ...
                if move_det:
                    for search_result_ship in self.env['account.move'].browse(move_det):
                        _logger.info('-----search_result_ship.amount_val************ = %s',search_result_ship.amount_val)
                        transaction_ref=search_result_ship.ref 
                        _logger.info('-----transaction_ref************ = %s',transaction_ref)
                        if transaction_ref.startswith( 'BKDP' ):
                            total_paid=total_paid + search_result_ship.amount_val
                        _logger.info('------before -total_paid************ = %s',total_paid)
                        reverse_name="reversal of: "+search_result_ship.name
                        move_det_reverse = self.env['account.move'].search(['&','&',('ref','=',reverse_name),('partner', '=', partner.id),('state', '=', 'posted')]).ids
                ...
                _logger.info('-------total_paid************ = %s',total_paid)    
                customer_deposits = self.env['bank.deposit'].search([('partner','=',partner.id),('state', '=', 'posted')])  
                # for customer_deposit in customer_deposits:
                #     total_bank_deposit=total_bank_deposit+customer_deposit.amount
                
                total_invo=total_paid - partner.total_invoiced
                _logger.info('-------total_paid************ = %s',total_paid)
                _logger.info('-------partner.total_invoiced************ = %s',partner.total_invoiced)
                total_invo = total_invo + inital_balance 
                partner.current_balance=total_invo+total_bank_deposit
                current_total_balance=total_invo+total_bank_deposit
                # partner.write({'current_balance_display':current_total_balance})

            elif partner.supplier:
                total_trade_creditor_debit_amount=0
                total_trade_creditor_credit_amount=0

                total_trade_debitor_debit_amount=0
                total_trade_debitor_credit_amount=0
                total_balance=0
                # trade_dabitors = self.env['account.account'].search([('is_trade_debitor', '=', True)]).ids
                
                # if trade_dabitors:
                #     _logger.info('-------trade_dabitors************ = %s',trade_dabitors)
                #     for trade_dabitor in self.env['account.account'].browse(trade_dabitors):
                #         _logger.info('-------trade_dabitor************ = %s',trade_dabitor)
                trade_dabitor=partner.property_account_receivable_id
                account_moves = self.env['account.move'].search([('partner', '=', partner.id)]).ids
                if account_moves:
                    _logger.info('-------account_moves************ = %s',account_moves)
                    for account_move in self.env['account.move'].browse(account_moves):
                        
                        account_move_lines = self.env['account.move.line'].search(['&','&',('debit','>',0),('account_id', '=', trade_dabitor.id),('move_id', '=', account_move.id)]).ids
                        if account_move_lines:
                            _logger.info('-------account_move_lines************ = %s',account_move_lines)
                            for account_move_line in self.env['account.move.line'].browse(account_move_lines):
                                total_trade_debitor_debit_amount=total_trade_debitor_debit_amount + account_move_line.debit
                                _logger.info('-------total_trade_debitor_debit_amount************ = %s',total_trade_debitor_debit_amount)
                        account_move_lines2 = self.env['account.move.line'].search(['&','&',('credit','>',0),('account_id', '=', trade_dabitor.id),('move_id', '=', account_move.id)]).ids
                        if account_move_lines2:
                            _logger.info('-------account_move_lines************ = %s',account_move_lines2)
                            for account_move_line in self.env['account.move.line'].browse(account_move_lines2):
                                total_trade_debitor_credit_amount=total_trade_debitor_credit_amount + account_move_line.credit
                                _logger.info('-------total_trade_debitor_credit_amount************ = %s',total_trade_debitor_credit_amount)
                
                # trade_creditors = self.env['account.account'].search([('is_trade_creditor', '=', True)]).id
                # if trade_creditors:
                #     for trade_creditor in self.env['account.account'].browse(trade_creditors):
                #         _logger.info('-------trade_creditor************ = %s',trade_creditor)
                trade_creditor=partner.property_account_payable_id
                account_moves = self.env['account.move'].search([('partner', '=', partner.id)]).ids
                if account_moves:
                    for account_move in self.env['account.move'].browse(account_moves):
                        account_move_lines = self.env['account.move.line'].search(['&','&',('credit','>',0),('account_id', '=', trade_creditor.id),('move_id', '=', account_move.id)]).ids
                        _logger.info('-------account_move_lines************ = %s',account_move_lines)
                        if account_move_lines:
                            for account_move_line in self.env['account.move.line'].browse(account_move_lines):
                                total_trade_creditor_credit_amount= total_trade_creditor_credit_amount + account_move_line.credit
                        account_move_lines2 = self.env['account.move.line'].search(['&','&',('debit','>',0),('account_id', '=', trade_creditor.id),('move_id', '=', account_move.id)]).ids
                        _logger.info('-------account_move_lines2************ = %s',account_move_lines2)
                        if account_move_lines2:
                            for account_move_line in self.env['account.move.line'].browse(account_move_lines2):
                                total_trade_creditor_debit_amount= total_trade_creditor_debit_amount + account_move_line.debit

                customer_deposits = self.env['bank.deposit'].search([('partner','=',partner.id),('state', '=', 'posted')])  
                
                for customer_deposit in customer_deposits:
                    total_bank_deposit=total_bank_deposit+customer_deposit.amount 
                total_balance = partner.inital_balance + total_trade_debitor_debit_amount - total_trade_debitor_credit_amount + total_trade_creditor_credit_amount  - total_trade_creditor_debit_amount
                partner.current_balance=total_balance
                # partner.write({'current_balance_display':total_balance})
    ...
    def _invoice_total(self):
        ...
        for partner in self:
            total_ivoice=0.0
            cancel_ivoice=0.0
            account_invoices = self.env['account.invoice'].search(['&',('state','=','paid'),('partner_id', '=', partner.id)]).ids
                    
            _logger.info('-------account_invoices************ = %s',account_invoices)
            if account_invoices:
                for account_invoice in self.env['account.invoice'].browse(account_invoices):
                    total_ivoice=total_ivoice + account_invoice.amount_total
            account_cancel_invoices = self.env['account.invoice'].search(['&',('type','=','out_refund'),('state','=','open'),('partner_id', '=', partner.id)]).ids
                    
            _logger.info('-------account_cancel_invoices************ = %s',account_cancel_invoices)
            if account_cancel_invoices:
                for account_cancel_invoice in self.env['account.invoice'].browse(account_cancel_invoices):
                    cancel_ivoice=cancel_ivoice + account_cancel_invoice.amount_total



            partner.total_invoiced=total_ivoice - cancel_ivoice
    ...
    current_balance = fields.Monetary(compute='_get_curent_balance', string="Current Balance", type='integer')
    current_balance_display = fields.Monetary( string="Current Balance", type='integer')
    net_cash = fields.Monetary(compute='_get_net_cash', string="Net Cash", type='integer')
    total_deposit = fields.Monetary(compute='_get_total_deposit', string="Deposits", type='integer')
    ...
    property_account_receivable_id = fields.Many2one('account.account', company_dependent=True,
        ...
        required=True)
    pre_payment_account_id = fields.Many2one('account.account', company_dependent=True,
        string="Pre-Payment Account",
        help="This account will be used for pre-payment",required=True)
    ...
    def action_view_partner_invoices(self):
        ...
        return action
    @api.multi
    def action_view_partner_balance(self):
        self.ensure_one()
        action = self.env.ref('account.action_invoice_refund_out_tree').read()[0]
        action['domain'] = literal_eval(action['domain'])
        action['domain'].append(('partner_id', 'child_of', self.id))
        return action
    @api.multi
    def action_view_partner_deposit(self):
        # self.ensure_one()
        _logger.info('------- self.id.id************ = %s', self.id)
        action = self.env.ref('account.action_total_deposit').read()[0]
        action['domain'] = [('partner', '=', self.id)]
        # action['domain'].append(('ref','like','BKDP%'))
        return action
    @api.onchange('total_invoiced')
    def _onchange_country_id(self):
        self.current_balance=self.total_invoiced