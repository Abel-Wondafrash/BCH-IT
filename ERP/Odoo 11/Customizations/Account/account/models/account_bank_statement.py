...
class AccountBankStatement(models.Model):
    ...
    last_date=fields.Date(string='Last Date of Month',required=True, states={'confirm': [('readonly', True)]}, index=True, copy=False, default=fields.Date.context_today)
    

    def _getTransitPayment(self,data=None):
              
        transitAmount=0.0


        current_date1 = self.last_date
        ...
        # _logger.info(' ---------- current_date2= = %s',current_date)
        current_quarter = round((current_date.month - 1) / 3 + 1)
        _logger.info(' ---------- current_quarter= = %s',current_quarter)
        first_date = datetime(current_date.year, 3 * current_quarter - 1, 1)
        _logger.info(' ---------- first_date= = %s',first_date)
        last_date = datetime(current_date.year, 3 * current_quarter - 0, 1) + timedelta(days=-1)
        ...
        if account_moves:
            for account_move in self.env['account.move'].browse(account_moves):
                ...
                account_move_lines= self.env['account.move.line'].search(['&','&',('statement_id','=',False),('debit','>',0),('move_id','=',account_move.id)]).ids
                if account_move_lines:
                    for account_move_line in self.env['account.move.line'].browse(account_move_lines):
                        # _logger.info(' ---------- account_move_lines= = %s',account_move_line)
                        transitAmount= transitAmount + account_move_line.debit
        _logger.info(' ---------- transitAmount= = %s',transitAmount)
        return transitAmount

    def _getOutstanding(self,data=None):
        ...
        if account_moves:
            for account_move in self.env['account.move'].browse(account_moves):
                # _logger.info(' ---------- account_move= = %s',account_move)
                account_move_lines= self.env['account.move.line'].search(['&','&',('statement_id','=',False),('credit','>',0),('move_id','=',account_move.id)]).ids
                if account_move_lines:
                    for account_move_line in self.env['account.move.line'].browse(account_move_lines):
                        # _logger.info(' ---------- account_move_lines= = %s',account_move_line)
                        transitAmount= transitAmount + account_move_line.debit
        _logger.info(' ---------- outstanding= = %s',transitAmount)
        return transitAmount
...
class AccountBankStatementLine(models.Model):
    ...
    def _getAccount(self, data=None):
        account_name=""
        account_move_lines = self.env['account.move.line'].search([('statement_line_id','=',self.id)]).ids
        if account_move_lines:
            for account_move_line in self.env['account.move.line'].browse(account_move_lines):
                account_id = account_move_line.account_id
                accounts = self.env['account.account'].search([('id','=',account_id.id)]).ids
                if accounts:
                    for account in self.env['account.account'].browse(accounts):
                        account_name = account.name
                        account_name=str(account_name)+ '('+str(account_id)+')'
        return account_name
    
             

    def _getRef(self, data=None):
        move_id=""
        account_name=""
        account_move_lines = self.env['account.move.line'].search([('statement_line_id','=',self.id)]).ids
        if account_move_lines:
            for account_move_line in self.env['account.move.line'].browse(account_move_lines):
                move_id= account_move_line.move_id
                accounts = self.env['account.move'].search([('id','=',move_id.id)]).ids
                if accounts:
                    for account in self.env['account.move'].browse(accounts):
                        account_name = account.name
                        account_name=str(account_name)
        return account_name
    def _getDate(self, data=None):
        date=""
        account_move_lines = self.env['account.move.line'].search([('statement_line_id','=',self.id)]).ids
        if account_move_lines:
            for account_move_line in self.env['account.move.line'].browse(account_move_lines):
